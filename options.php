...
/**
 * Административная страница настроек модуля `sms.traffic`.
 *
...
$APPLICATION->SetTitle(Loc::getMessage('SMS_TRAFFIC_OPT_TITLE'));
require_once $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/main/include/prolog_admin_after.php';

$tabs = [
    [
        'DIV' => 'sms_traffic_main',
        'TAB' => Loc::getMessage('SMS_TRAFFIC_OPT_TAB_MAIN'),
        'TITLE' => Loc::getMessage('SMS_TRAFFIC_OPT_TAB_MAIN'),
    ],
];
...
<form method="post" action="<?= $APPLICATION->GetCurPage() ?>?mid=<?= htmlspecialcharsbx($mid) ?>&lang=<?= LANGUAGE_ID ?>">
    ...
    <tr>
        <td colspan="2"><?= Loc::getMessage('SMS_TRAFFIC_OPT_MAIN_MODULE_LINK') ?></td>
    </tr>
    <tr>
        <td width="40%"><?= Loc::getMessage('SMS_TRAFFIC_OPT_LOGIN') ?>:</td>
        <td><input type="text" name="login" value="<?= htmlspecialcharsbx($login) ?>" size="40"></td>
    </tr>
    <tr>
        <td><?= Loc::getMessage('SMS_TRAFFIC_OPT_PASSWORD') ?>:</td>
        <td><input type="password" name="password" value="<?= htmlspecialcharsbx($password) ?>" size="40" autocomplete="new-password"></td>
    </tr>
    <tr>
        <td class="adm-detail-valign-top"><?= Loc::getMessage('SMS_TRAFFIC_OPT_ORIGINATORS') ?>:</td>
        <td>
            <textarea name="originators" rows="4" cols="50"><?= htmlspecialcharsbx($originators) ?></textarea>
            <div class="adm-info-message-wrap"><div class="adm-info-message"><?= Loc::getMessage('SMS_TRAFFIC_OPT_ORIGINATORS_HINT') ?></div></div>
        </td>
    </tr>
    <tr>
        <td><?= Loc::getMessage('SMS_TRAFFIC_OPT_RUS') ?>:</td>
        <td>
            <select name="rus">
